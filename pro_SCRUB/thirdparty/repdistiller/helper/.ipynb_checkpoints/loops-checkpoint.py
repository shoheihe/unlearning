        ...
        if not quiet:
            if split == "minimize":
                if idx % opt.print_freq == 0:
                    ...
                    sys.stdout.flush()
    if split=="maximize":            
        if epoch==2 or epoch==10:
            print('*'*50)
            print(f'epoch:{epoch}')
            '''
            prediction=torch.nn.functional.softmax(logit_s)
            print(f'logit_s:{prediction}')
            max_val,max_index=torch.max(prediction, axis=1)
            print(f'prediction_labal:{max_index}')
            print(f'taergets:{train_loader.dataset.targets}')
            print('*'*50)'''
